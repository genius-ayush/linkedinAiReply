export default defineContentScript({
  ...
  main() {
    ...
    // Function to add the AI icon when the input is found
    function addAIIcon(messageInput: HTMLElement) {
      console.log('Input field found:', messageInput);
      
      // Create the AI icon
      const aiIcon = document.createElement('div');
      aiIcon.innerHTML = `<svg width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      aiIcon.style.position = 'absolute'; // Adjust position as necessary
      aiIcon.style.right = '10px';
      aiIcon.style.top = '10px';
      aiIcon.style.cursor = 'pointer';

      // Append the AI icon to the parent of the message input
    ...
    // Use MutationObserver to wait for LinkedIn's dynamic content
